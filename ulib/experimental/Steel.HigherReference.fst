...
let ghost_pts_to_witinv (#a:Type) (r:ghost_ref a) (p:perm)
  ...
  introduce forall x y m. interp ((ghost_pts_to_sl r p) x) m /\ interp ((ghost_pts_to_sl r p) y) m ==> x == y
  with introduce interp ((ghost_pts_to_sl r p) x) m /\ interp ((ghost_pts_to_sl r p) y) m ==> x == y
       with _. Mem.pts_to_join (Ghost.reveal r) (Some (Ghost.reveal x, p)) (Some (Ghost.reveal y, p)) m
